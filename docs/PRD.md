...
- Before cell selection is enabled, the participant must confirm the active mode, acknowledge the current safety notice, and complete camera permission plus basic framing checks when camera mode is selected. The camera ready-position check occurs after the pixel is reserved, so the participant can place the pixel first and then move into plank position. Guided demo uses an isolated simulated canvas and never reserves a real cell.
...
- A successful reservation enters a `pending` state and makes the coordinate temporarily unavailable to other participants. In camera mode, the timer remains at zero in a `positioning` state until on-device pose validation detects the supported ready position; that first ready result starts the standard three-second countdown. In honor mode, the countdown starts immediately after reservation. The participant’s pending pixel pulses on the canvas and is labelled `YOUR PIXEL · PENDING` throughout positioning, countdown, and the active attempt.
...
- Camera mode keeps a fixed-height five-cell feedback block visible across ready, positioning, countdown, active, failure, and completion states to prevent layout shifts. It shows `GET IN POSITION` while waiting for the ready pose, `HOLD READY` during the three-second countdown, `PERFECT FORM 🔥` while valid time is credited, `HIPS UP` or `HIPS DOWN` during correction, `HEY, COME BACK!` during tracking loss, `POWER UP +2` on completion, and `TOO BAD!` after an abandoned attempt. Correction states highlight the affected body region and place a short, oversized pixel-art directional arrow directly above the form-grace status text; no duplicate corrective text overlays the character. At-a-glance corrective copy uses three words or fewer whenever possible. Honor mode remains labelled `FORM NOT CAMERA-VALIDATED` and does not show camera form feedback.
- Completion plays a one-shot three-frame celebration from the shared sprite-state atlas, progressing from kneeling recovery to celebration and ending on the raised-fists pose. Atlas crops share one bottom-center viewport, advance every 160 ms, and then hold the final pose; reduced-motion preferences skip directly to that final frame.
...
5. For camera validation, grant permission, receive local-only pose detection, and pass basic framing checks so the required landmarks can be evaluated.
...
7. Select an available canvas coordinate to atomically reserve it. A collision asks the participant to choose another cell without consuming entitlement.
...
10. In camera mode, move into the supported ready position and hold it to trigger the three-second countdown; in honor mode, begin the countdown immediately after reservation. Start crediting time after that countdown according to the selected route: valid detected form for camera mode or continuous active-page time for honor mode.
...
- Implement camera setup, required-landmark guidance, the post-reservation ready-position gate, countdown, unified vertical active main page, timer, retries, abandonment, and automatic commit or release transitions.
...
| 2026-07-16 | MVP 1 uses a pixel-first challenge: after route, safety, and readiness requirements pass, selecting an available canvas cell atomically reserves it and triggers the three-second countdown; there is no separate start or post-completion placement action. | Superseded on 2026-07-17 for camera-mode readiness timing |
...
| 2026-07-17 | Camera mode reserves the selected pixel first, waits with the timer at zero until on-device pose validation detects the ready position, and only then starts the three-second countdown. Honor mode still starts the countdown immediately after reservation. | Confirmed |
| 2026-07-17 | Camera corrections use a highlighted body region, an oversized directional arrow, and direction-first copy of three words or fewer when possible, including `HIPS UP` and `HIPS DOWN`. | Confirmed |
