...
Plank As One turns a short plank into a shared daily ritual: each person completes one safe plank challenge, earns one pixel, and helps reveal the community-built **PLANK AS ONE** artwork. The MVP is a focused SvelteKit experience with mock presence, on-device MoveNet camera validation, safety guidance, audio cues, and pixel reservation rules.
...
The shared canvas and presence data are mocked locally. Camera mode loads TensorFlow.js and MoveNet only after a pixel is selected; camera access requires a secure context such as `localhost` or HTTPS.
...
4. In Camera mode, allow access, follow the side-view framing guide, and hold the plank for the displayed duration. In a development build, the floating **DEV TOOLS** panel can simulate valid form, low hips, high hips, and tracking loss; it is excluded from production builds.
...
- `apps/web/src/lib/pose/` — lazy MoveNet detector, frame scheduler, normalized geometry, smoothing, rules, correction arbitration, setup framing, and audio feedback
...
- `apps/web/test/pose-engine.test.mjs` — deterministic geometry and pose-rule fixtures
