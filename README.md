...
With `PUBLIC_ENABLE_POSE_DEV_TOOLS=true` in a development build, **DEV TOOLS** can simulate readiness, valid form, low hips, high hips, tracking loss, and completion. **POSE VISUAL LOG** can also save consented, annotated camera frames and paired JSON diagnostics to a local folder for camera reliability testing. The flag defaults to false, and both tools remain unavailable in production builds even if it is set. See [`docs/POSE_DEBUG_MODE.md`](./docs/POSE_DEBUG_MODE.md) for the capture workflow and privacy requirements.

The workout character uses individually annotated sprite states extracted from a shared atlas. The runtime loads each sprite's adjacent 480×480 JSON file, then normalizes live on-device landmarks and matches them to the nearest compatible frame with switch stabilization. Unavailable annotations fall back to deterministic states. See [`docs/SPRITE_STATE_AVATARS.md`](./docs/SPRITE_STATE_AVATARS.md) for the annotation and export workflow.
