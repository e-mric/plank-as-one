...
export function createPoseDebugRecord({ pose, result, stage, capturedAt = new Date(), frameTimeMs = null, width = 1, height = 1, reason = 'automatic' }) {
  ...
  return {
    ...
    quality: {
      ...
      selectedMeanScore: round(quality.selectedMeanScore, 3),
      visibleLandmarkCount: quality.visibleLandmarkCount ?? 0,
      supportedLandmarkCount: quality.supportedLandmarkCount ?? 0,
      transientLoss: Boolean(quality.transientLoss),
...
export function drawPoseDebugOverlay({ canvas, video, pose, result, stage, capturedAt = new Date(), frameTimeMs = null, reason = 'automatic' }) {
  ...
  const lines = [
    ...
    `POINTS ${record.quality.visibleLandmarkCount}/6${record.quality.transientLoss ? ' / GRACE' : ''}`,
